#!/usr/bin/env python3
"""
WeeWX OpenWeather Extension Installer - Fixed Database Field Creation

Only fixes the specific technical errors without changing field selection logic.
...
class DatabaseManager:
    ...
    def _add_missing_fields(self, missing_fields, field_mappings):
        ...
        # FIX 1: Get config file path correctly from ConfigObj
        if hasattr(self.config_dict, 'filename') and self.config_dict.filename:
            config_path = self.config_dict.filename
        else:
        ...
        for field_name in sorted(missing_fields):
            ...
            try:
                ...
                # FIX 2: Correct weectl command syntax - separate --config and path
                cmd = [weectl_path, 'database', 'add-column', field_name, 
                       '--config', config_path, '-y']
                
                # Only add --type for REAL/INTEGER (weectl limitation)
                if field_type in ['REAL', 'INTEGER']:
                    cmd.insert(-1, '--type')  # Insert before -y
                    cmd.insert(-1, field_type)
...
class OpenWeatherInstaller(ExtensionInstaller):
    ...
    def __init__(self):
        super(OpenWeatherInstaller, self).__init__(
            ...
            config={
                ...
                # Manual service registration - NO Engine section in config
            }
        )
    ...
    def configure(self, engine):
        ...
        try:
            ...
            # Step 7: Manual service registration (like AirVisual)
    ...
    def _register_service(self, config_dict):
        """Manual service registration following AirVisual pattern."""
        print("\n" + "="*60)
